...
import android.view.ViewGrouppackage ua.lviv.maf
...
class PredictionsFragment : Fragment() {
    ...
    // 🔥 ОНОВЛЕНО: Відображаємо ім'я та запускаємо завантаження матчів
    ...
    private fun checkAuthState() {
        ...
        if (isLogged) {
            ...
            // Якщо авторизований - вантажимо матчі
    ...
    // 🔥 ДОДАНО: Завантаження реальних матчів з БД сайту
                    ...
                    if (json.optString("status") == "success") {
                        ...
                        activity?.runOnUiThread {
                            ...
                            rvPredictions.adapter = PredictionAdapter(matchesList) { match, score1, score2 ->
                                // ТУТ БУДЕ ВІДПРАВКА ПРОГНОЗУ НА СЕРВЕР
